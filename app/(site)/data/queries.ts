...
// Document query that accepts a single object with optional parameters
export const GENERATE_DOC_QUERY = ({
  documentType,
  filter = "",
  fields = "...",
  limit = "",
  ...
  if (!documentType) {
    throw new Error("documentType is required for GENERATE_DOC_QUERY");
  }

  return groq`*[_type == "${documentType}" ${filter}] ${order} ${limit} {
    ${fields}
  }`;
};
